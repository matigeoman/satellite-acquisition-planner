...
def test_orbit_page_is_registered() -> None:
    ...
    assert 'ORBITS = "Orbity publiczne"' in navigation
...
def test_large_interface_scale_and_map_controls_are_present() -> None:
    stylesheet = Path("app/ui/assets/application.css").read_text(
        encoding="utf-8"
    )
    aoi_editor = Path("app/ui/components/aoi_editor.py").read_text(
        encoding="utf-8"
    )
