...
def main() -> None:
    ...
    stress_request_set = load_request_set(REQUEST_SET_PATH)
    ...
    print()
    print(f"Limit CP-SAT: {CP_SAT_TIME_LIMIT_S:g} s")
    print(f"Rozmiary: {REQUEST_COUNTS}")
    print()

    for request_count in REQUEST_COUNTS:
        print(f"SCENARIUSZ — {request_count} ZLECEŃ")
        ...
        opportunity_count = len(opportunity_set.feasible_opportunities)

        selection_variable_count = opportunity_count

        request_variable_count = len(request_set.active_requests)
        ...
        greedy_schedule = build_greedy_schedule(
            ...
            schedule_id=(f"SCHEDULE-SCALE-GREEDY-{request_count:03d}"),
            name=(f"Benchmark skalowalności Greedy — {request_count} zleceń"),
            created_at_utc=created_at,
        )

        greedy_path = BENCHMARK_DIRECTORY / (f"scale_{request_count:03d}_greedy.json")
        ...
        greedy_result = ScalabilityRunResult(
            ...
            selection_variable_count=(selection_variable_count),
            request_variable_count=(request_variable_count),
            auxiliary_variable_count=(auxiliary_variable_count),
            algorithm="GREEDY",
            time_limit_s=None,
            solver_status="NOT_APPLICABLE",
            schedule_status=(greedy_schedule.status.value),
            objective_value=float(greedy_schedule.objective_value or 0.0),
            fully_satisfied_requests=(greedy_analysis.fully_satisfied_requests),
            unassigned_requests=(greedy_analysis.unassigned_requests),
            satisfaction_ratio=(greedy_analysis.satisfaction_ratio),
            total_acquisitions=(greedy_analysis.total_acquisitions),
            runtime_s=float(greedy_schedule.solver_runtime_s or 0.0),
            schedule_path=str(greedy_path),
        )
        ...
        cp_sat_schedule = cp_sat_scheduler.build_schedule(
            schedule_id=(f"SCHEDULE-SCALE-CP-SAT-{request_count:03d}"),
            name=(f"Benchmark skalowalności CP-SAT — {request_count} zleceń"),
            created_at_utc=created_at,
        )

        cp_sat_path = BENCHMARK_DIRECTORY / (f"scale_{request_count:03d}_cp_sat.json")
        ...
        cp_sat_result = ScalabilityRunResult(
            ...
            solver_status=(cp_sat_scheduler.last_solver_status or "UNKNOWN"),
            schedule_status=(cp_sat_schedule.status.value),
            objective_value=float(cp_sat_schedule.objective_value or 0.0),
            fully_satisfied_requests=(cp_sat_analysis.fully_satisfied_requests),
            unassigned_requests=(cp_sat_analysis.unassigned_requests),
            satisfaction_ratio=(cp_sat_analysis.satisfaction_ratio),
            total_acquisitions=(cp_sat_analysis.total_acquisitions),
            runtime_s=float(cp_sat_schedule.solver_runtime_s or 0.0),
            schedule_path=str(cp_sat_path),
        )

        results.append(cp_sat_result)

        if greedy_result.objective_value > 0.0:
            improvement_pct = (
                (cp_sat_result.objective_value - greedy_result.objective_value)
        ...
        print(f"  Poprawa celu: {improvement_pct:.2f}%")
        print(
