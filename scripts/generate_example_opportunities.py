...
def select_mode(
    ...
    if not candidates:
        raise ValueError(
            f"Brak trybu dla zlecenia {request.request_id} i sensora {sensor.sensor_id}"
        )
...
def calculate_duration_s(
    ...
    duration = (
        ...
        + (mode.max_acquisition_duration_s - mode.min_acquisition_duration_s) * ratio
    )
...
def calculate_time_window(
    ...
    center = request.earliest_start_utc + timedelta(
    ...
    start_utc = center - timedelta(seconds=duration_s / 2.0)

    latest_possible_start = request.latest_end_utc - timedelta(seconds=duration_s)
...
def build_observation_parameters(
    *,
    ...
    if deliberately_infeasible:
        ...
        cloud_limit = (
            request.max_cloud_cover if request.max_cloud_cover is not None else 0.3
        )
    ...
    if sensor_type == SensorType.SAR:
        ...
        incidence_angle_deg = round(
            min(
                75.0,
                off_nadir_angle_deg + rng.uniform(3.0, 9.0),
            ),
            3,
        )

        observation_side = (
            ObservationSide.LEFT if local_index % 2 == 0 else ObservationSide.RIGHT
        )
    ...
    observation_side = side_sequence[local_index % len(side_sequence)]
...
def determine_infeasibility_reasons(
    *,
    ...
    off_nadir_angle_deg = parameters["off_nadir_angle_deg"]

    if mode.nominal_resolution_m > request.max_resolution_m:
        reasons.append("Niewystarczająca rozdzielczość trybu")

    if coverage_ratio < request.minimum_coverage_ratio:
        reasons.append("Niewystarczające pokrycie celu")

    if off_nadir_angle_deg > mode.max_off_nadir_deg:
        reasons.append("Przekroczony limit off-nadir trybu")

    if (
        request.max_off_nadir_deg is not None
        and off_nadir_angle_deg > request.max_off_nadir_deg
    ):
        reasons.append("Przekroczony limit off-nadir zlecenia")
    ...
    if sensor.sensor_type == SensorType.SAR:
        ...
        if (
            ...
            and incidence_angle_deg < mode.min_incidence_angle_deg
        ):
            reasons.append("Kąt padania poniżej minimum trybu")

        if (
            mode.max_incidence_angle_deg is not None
            and incidence_angle_deg > mode.max_incidence_angle_deg
        ):
            reasons.append("Kąt padania powyżej maksimum trybu")

        if (
            request.max_incidence_angle_deg is not None
            and incidence_angle_deg > request.max_incidence_angle_deg
        ):
            reasons.append("Przekroczony limit kąta padania zlecenia")

    else:
        cloud_cover = parameters["cloud_cover"]
        sun_elevation_deg = parameters["sun_elevation_deg"]

        if (
            request.max_cloud_cover is not None
            and cloud_cover > request.max_cloud_cover
        ):
            reasons.append("Przekroczony limit zachmurzenia")

        if (
            sensor.minimum_sun_elevation_deg is not None
            and sun_elevation_deg < sensor.minimum_sun_elevation_deg
        ):
            reasons.append("Elewacja Słońca poniżej minimum")
...
def calculate_quality_score(
    *,
    ...
    score = mode.quality_factor * coverage_ratio * angle_factor * weather_factor
...
def build_opportunity_set(
    ...
    satellites_by_type = {
        SensorType.SAR: sorted(
            [
                ...
                if catalog.get_sensor(satellite.sensor_id).sensor_type == SensorType.SAR
            ],
        ...
        SensorType.OPTICAL: sorted(
            [
                satellite
                for satellite in catalog.active_satellites
                if catalog.get_sensor(satellite.sensor_id).sensor_type
                == SensorType.OPTICAL
            ],
            key=lambda satellite: satellite.satellite_id,
        ),
    ...
    for request_index, request in enumerate(request_set.active_requests):
        ...
        for sensor_type, count in generation_plan:
            ...
            for local_index in range(count):
                satellite = satellites[(request_index + local_index) % len(satellites)]
                ...
                estimated_data_volume_mb = round(
                    mode.data_rate_mb_s * duration_s,
                    6,
                )

                sensor_label = "SAR" if sensor_type == SensorType.SAR else "EO"

                opportunity_id = f"OPP-{sensor_label}-{global_counter:04d}"
                ...
                opportunities.append(
                    {
                        ...
                        "start_utc": start_utc,
                        "end_utc": end_utc,
                        "observation_side": parameters["observation_side"].value,
                        "off_nadir_angle_deg": parameters["off_nadir_angle_deg"],
                        "incidence_angle_deg": parameters["incidence_angle_deg"],
                        "cloud_cover": parameters["cloud_cover"],
                        "sun_elevation_deg": parameters["sun_elevation_deg"],
...
def main() -> None:
    ...
    print(f"Zapisano {len(opportunity_set.opportunities)} okazji do:")
    print(OUTPUT_PATH)
